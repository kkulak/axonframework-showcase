...
case class FailureAttemptAmountSchedulingStrategy(scheduler: Scheduler, maxAmount: Integer) extends RequestSchedulingStrategy {
  ...
  override def schedule(implicit ref: ActorRef, ctx: ExecutionContext): Unit = scheduler.scheduleOnce(5 seconds, ref, SendRequestCommand)
}
